...
export default function Navbar({ cartCount = 0, wishlistCount = 0, onCartClick, onWishlistClick }) {
  ...
  const fetchLiveCounts = async () => {
    ...
    try {
      ...
      if (wishRes.ok && Array.isArray(wishRes.data)) {
        setLiveWishlistCount(wishRes.data.length);
      }
      if (cartRes.ok && typeof cartRes.data?.count === 'number') {
        setLiveCartCount(cartRes.data.count);
      } else if (cartRes.ok && typeof cartRes.data === 'number') {
        setLiveCartCount(cartRes.data);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const handleWishlistUpdate = () => fetchLiveCounts();
    const handleCartUpdate = () => fetchLiveCounts();
  ...
  const displayWishlistCount = wishlistCount > 0 ? wishlistCount : liveWishlistCount;
  const displayCartCount = cartCount > 0 ? cartCount : liveCartCount;
